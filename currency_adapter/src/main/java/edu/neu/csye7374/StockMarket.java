...
//    }
    ...
    // Trade stock object
    public void tradeStock(String stock, double bid) throws Exception {
        System.out.println("Trading " + stock + " stock for bid $" + bid + " on the stock market");
        Iterator stockArr = stocks.iterator();
        while (stockArr.hasNext()) {
            Stock s = (Stock) stockArr.next();
    ...
    public static void adapterDemo() throws Exception {
        ...
        StockMarketClassAdapter stockMarketClassAdapter = new StockMarketClassAdapter();
        StockMarketObjectAdapter stockMarketObjectAdapter = new StockMarketObjectAdapter(stockMarket);

        stockMarketClassAdapter.addStock(netflixStock);
        stockMarketClassAdapter.addStock(spotifyStock);
        stockMarketClassAdapter.addStock(amazonStock);

        stockMarket.addStock(netflixStock);
        stockMarket.addStock(spotifyStock);
        stockMarket.addStock(amazonStock);

        System.out.println("\n********* Trading using 'Class Adapter' *********\n");
        stockMarketClassAdapter.tradeStock("NFLX", new Currency(CurrencyType.USD, 75.75));
        stockMarketClassAdapter.tradeStock("SPOT", new Currency(CurrencyType.CAD, 85.50));
        stockMarketClassAdapter.tradeStock("AMZN", new Currency(CurrencyType.NGN, 100000.00));

        System.out.println("\n********* Trading using 'Object Adapter' *********\n");
        stockMarketObjectAdapter.tradeStock("NFLX", new Currency(CurrencyType.USD, 120.50));
        stockMarketObjectAdapter.tradeStock("SPOT", new Currency(CurrencyType.CAD, 170.75));
        stockMarketObjectAdapter.tradeStock("AMZN", new Currency(CurrencyType.NGN, 200000.00));
